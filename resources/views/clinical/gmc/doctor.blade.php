...
@section('title', $pageTitle ?? 'GMC - Doctor View')

@push('styles')
<style>
    /* ── Teal / Emerald theme ──────────────────────────────────────────── */
    :root { --c:#0d9488; --c-light:#f0fdfa; --c-mid:#99f6e4; --c-dark:#0f766e; }
    ...
    .gmc-card-header { background:#f6fffe !important; border-bottom:1px solid #d0f5f0 !important;
    ...
    .search-wrap .form-control:focus { border-color:var(--c); box-shadow:0 0 0 .2rem rgba(13,148,136,.15); }
    .filter-row .form-select, .filter-row .form-control { font-size:.82rem; border-color:#e2e8f0; }
    .filter-row .form-select:focus, .filter-row .form-control:focus { border-color:var(--c); box-shadow:0 0 0 .2rem rgba(13,148,136,.12); }
    ...
    /* Doctor: hoverable queue rows */
    .queue-patient-row[data-href]:hover { background:#f0fdfa !important; }
    ...
    <div class="d-flex align-items-center justify-content-between position-relative" style="z-index:1;">
        <div class="d-flex align-items-center gap-3">
            <div class="rounded-3 d-flex align-items-center justify-content-center flex-shrink-0"
                ...
                <i class="bi bi-person-badge-fill" style="font-size:1.45rem;color:white;"></i>
            </div>
            <div>
        ...
        <div class="text-end flex-shrink-0 position-relative" style="z-index:1;">
            <div class="fw-bold text-white font-monospace" id="live-clock"
                 style="font-size:1.35rem;letter-spacing:.04em;">--:--:--</div>
            <div class="small text-white" style="opacity:.75;">{{ now()->format('l, d F Y') }}</div>
        </div>
    </div>
</div>
        ...
        <div class="modal-content border-0">
            ...
            <div class="modal-body px-4 py-3">
                <div class="rounded-2 px-3 py-2 mb-3 d-flex align-items-center gap-2"
                     style="background:var(--c-light);border:1px solid var(--c-mid);">
                ...
                <div class="row g-2 mb-3" id="modal-category-group">
                    <div class="col-6">
                        <input type="radio" class="btn-check" name="modal_category" id="mcat-opd" value="opd">
                        <label class="btn btn-outline-primary w-100 text-start py-2 px-3 small" for="mcat-opd">
                            <i class="bi bi-hospital me-2"></i>OPD
                        </label>
                    </div>
                    <div class="col-6">
                        <input type="radio" class="btn-check" name="modal_category" id="mcat-new-clinic" value="new_clinic_visit">
                        <label class="btn btn-outline-danger w-100 text-start py-2 px-3 small" for="mcat-new-clinic">
                            <i class="bi bi-person-plus-fill me-2"></i>New Clinic Visit
                        </label>
                    </div>
                    <div class="col-6">
                        <input type="radio" class="btn-check" name="modal_category" id="mcat-recurrent-clinic" value="recurrent_clinic_visit">
                        <label class="btn btn-outline-primary w-100 text-start py-2 px-3 small" for="mcat-recurrent-clinic">
                            <i class="bi bi-arrow-repeat me-2"></i>Recurrent Visit
                        </label>
                    </div>
                    <div class="col-6">
                        <input type="radio" class="btn-check" name="modal_category" id="mcat-urgent" value="urgent">
                        <label class="btn btn-outline-warning w-100 text-start py-2 px-3 small" for="mcat-urgent">
                            <i class="bi bi-person-badge-fill me-2"></i>Urgent
                        </label>
                    </div>
                </div>
                <div id="modal-cat-error" class="text-danger small mb-2" style="display:none;">Please select a category.</div>
                <hr class="my-2">
                <div class="row g-2 mb-2">
                    <div class="col-12">
                        <label class="form-label form-label-sm mb-1">OPD Number</label>
                        <input type="text" id="modal-opd-number" class="form-control form-control-sm" placeholder="OPD number" maxlength="20">
                    </div>
                </div>
                <div class="row g-2 mb-2">
                    <div class="col-6">
                        <label class="form-label form-label-sm mb-1">Height (cm)</label>
                        <input type="number" id="modal-height" class="form-control form-control-sm" placeholder="e.g. 165" min="1" max="300" step="0.1">
                    </div>
                    <div class="col-6">
                        <label class="form-label form-label-sm mb-1">Weight (kg)</label>
                        <input type="number" id="modal-weight" class="form-control form-control-sm" placeholder="e.g. 68" min="1" max="500" step="0.1">
                    </div>
                </div>
                <div class="mb-0">
                    ...
                    <div class="row g-2 mb-1">
                        <div class="col-6">
                            <input type="number" id="modal-bp-systolic" class="form-control form-control-sm" min="0" max="300">
                        </div>
                        <div class="col-6">
                            <input type="number" id="modal-bp-diastolic" class="form-control form-control-sm" min="0" max="300">
                        </div>
                    </div>
...
<script>
(function () {
    const listUrl   = "{{ route('clinical.patients.list', $unitView->id) }}";
    ...
    function bindQueueTabs() {
        ...
        // Doctor: queue patient rows are clickable
    ...
    document.getElementById('addToQueueModal').addEventListener('hidden.bs.modal', function () {
        document.getElementById('modal-opd-number').value     = '';
    ...
    window.openAddToQueueModal = function (btn) {
        ...
        const newClinicWrapper = document.getElementById('mcat-new-clinic').closest('.col-6');
        if (btn.dataset.hasClinicVisit === '1') {
            newClinicWrapper.style.display = 'none';
            document.getElementById('mcat-new-clinic').checked = false;
        } else {
    ...
    modalConfirmBtn.addEventListener('click', function () {
        ...
        const opdNum       = document.getElementById('modal-opd-number').value.trim();
        const height       = document.getElementById('modal-height').value.trim();
        ...
        if (opdNum)       body.append('opd_number',      opdNum);
