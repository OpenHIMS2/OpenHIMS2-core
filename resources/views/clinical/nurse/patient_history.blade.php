...
.profile-tabs .nav-link:not(.active):hover { background: #f0fdfa; border-color: #99f6e4; color: #0f766e; }
...
.complaint-bar-fill  { height:100%; border-radius:2rem; background:linear-gradient(90deg,#0f766e,#0891b2); }

/* ── Visit history sidebar ───────────────────────────────────────────────── */
.visit-item { transition: background .12s; }
.visit-item:hover { background: #f0fdfa; }
.visit-item.active { background: #f0fdfa; border-left: 3px solid #0f766e; }
...
{{-- ══════════════════════════════════════════════════════════════════════ --}}
{{-- TODAY'S VISIT — status only, no action buttons                        --}}
{{-- ══════════════════════════════════════════════════════════════════════ --}}
@if($todayVisit)
        ...
        <div class="d-flex align-items-center gap-3">
            ...
            @elseif($todayVisit->status === 'in_progress')
                <span class="badge bg-warning-subtle text-warning border border-warning-subtle px-2 py-1">
                    <span class="blink-dot text-warning me-1">●</span>In Progress
                </span>
            @endif
        </div>
    </div>
        ...
        <button class="nav-link" id="visits-tab" data-bs-toggle="tab"
            ...
            @if($pastVisits->isNotEmpty())
                <span class="badge rounded-pill ms-1"
                      style="background:#0f766e;font-size:.62rem;">{{ $pastVisits->count() }}</span>
            @endif
        </button>
    </li>
</ul>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-sm mb-0 inv-table">
                    <thead>
                        <tr>
                            <th class="px-3 py-2">Investigation</th>
                            <th class="py-2" style="width:12%">Value</th>
                            <th class="py-2" style="width:18%">Date</th>
                            <th class="py-2" style="width:10%">Visit</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php $invGroups = $allInvs->groupBy('name'); @endphp
                        @foreach($invGroups as $invName => $rows)
                            @foreach($rows as $idx => $inv)
                            <tr>
                                @if($idx === 0)
                                <td class="px-3 fw-semibold" rowspan="{{ $rows->count() }}">{{ $invName }}</td>
                                @endif
                                ...
                                <td>
                                    <span class="badge" style="background:#f0f9ff;color:#0369a1;font-size:.68rem;">
                                        #{{ $inv['visit_no'] }}
                                    </span>
                                </td>
                            </tr>
            ...
            <div class="card section-card sidebar-sticky">
                <div class="card-header d-flex align-items-center justify-content-between">
                    ...
                    <span class="badge rounded-pill"
                          style="background:#0f766e;color:#fff;font-size:.65rem;">
                        {{ $pastVisits->count() }}
                    </span>
                </div>
                <div class="list-group list-group-flush" style="max-height:70vh;overflow-y:auto;">
                    @foreach($pastVisits as $v)
                        ...
                        @endphp
                        <a href="{{ route('clinical.nurse.patient-history', [$unitView->id, $patient->id]) }}?visit={{ $v->id }}#visits-tab"
                           ...
                           onclick="document.getElementById('visits-tab').click()">
                            <div class="d-flex align-items-center gap-2">
                                <span class="rounded-circle d-flex align-items-center justify-content-center fw-bold flex-shrink-0"
                                      style="width:2rem;height:2rem;font-size:.7rem;background:{{ $isActive ? '#ccfbf1' : '#f1f5f9' }};color:{{ $isActive ? '#0f766e' : '#64748b' }};">
                                    #{{ $v->visit_number }}
                                </span>
