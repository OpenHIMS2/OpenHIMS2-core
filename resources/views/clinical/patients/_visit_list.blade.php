...
@else
    @php
        ...
        $isDocView   = str_contains($bladePath, 'doctor');
    @endphp
                            ...
                            @if($isDocView)
                                <a href="{{ route('clinical.doctor.patient-history', [$unitView->id, $visit->patient_id]) }}"
                                   ...
                                   title="Open patient profile">
                                    {{ $visit->patient->name }}
                                    <i class="bi bi-arrow-up-right-circle ms-1 text-muted" style="font-size:.7rem;"></i>
                                </a>
                            @else
                                {{ $visit->patient->name }}
                            @endif
                            ...
                            <div class="d-flex align-items-center gap-1">
                                ...
                                @if(!in_array($visit->patient_id, $inQueueToday))
                                    <button type="button"
                                            ...
                                            data-clinic-number="{{ $clinicNumbers[$visit->patient_id] ?? '' }}"
